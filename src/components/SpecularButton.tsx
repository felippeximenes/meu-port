import { useRef, useEffect, type MouseEventHandler, type ReactNode, type CSSProperties } from 'react';
...
export default function SpecularButton({
  ...
  useEffect(() => {
    ...
    const update = (now: number) => {
      ...
      lineC.set(p.lineColor as any);
      baseC.set(p.baseColor as any);
  ...
  if (href) {
    return (
      <a ref={btnRef as React.Ref<HTMLAnchorElement>} href={href} target={target} rel={rel} className={cls} style={cssVars} onClick={onClick as any}>
        {inner}
      </a>
    );
  }
  ...
  return (
    <button ref={btnRef as React.Ref<HTMLButtonElement>} type={type} disabled={disabled} onClick={onClick} className={cls} style={cssVars}>
      {inner}
    </button>
  );
}
