...
const mediaBox = { borderRadius: 16, overflow: 'hidden', border: '1px solid rgba(0,0,0,0.06)', boxShadow: '0 2px 4px rgba(0,0,0,0.04), 0 8px 24px rgba(0,0,0,0.05)', background: '#EEECEA' } as const;

function ProjectCardDesktop({ p }: { p: Project }) {
  const [imgErr, setImgErr] = useState(false);
  return (
    <div style={{ height: '100%', display: 'grid', gridTemplateColumns: '1fr 1.2fr', background: '#F8F7F5', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 3vw 0 5vw' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <span style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--purple)' }}>{p.n} — {NUM.toString().padStart(2, '0')}</span>
          ...
          <div style={{ marginTop: 8 }}><SpecularButton href={p.href} target="_blank" rel="noreferrer" size="sm">Ver projeto ↗</SpecularButton></div>
        </div>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '36px 36px 36px 0', gap: 10 }}>
        <div style={{ ...mediaBox, aspectRatio: '16 / 9' }}>
          {p.video ? <video src={p.video} autoPlay={!window.matchMedia('(prefers-reduced-motion: reduce)').matches} muted loop playsInline style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'left top', display: 'block' }} /> : imgErr ? <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted)', fontSize: 13 }}>{p.name}</div> : <img src={p.img} alt={p.name} onError={() => setImgErr(true)} style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'left top', display: 'block' }} />}
        </div>
...
function ProjectCardMobile({ p }: { p: Project }) {
  return (
    <div style={{ background: '#F8F7F5', borderRadius: 20, overflow: 'hidden', border: '1px solid rgba(0,0,0,0.06)', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        <span style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--purple)' }}>{p.n} — {NUM.toString().padStart(2, '0')}</span>
        <h3 style={{ margin: 0, fontFamily: "'Inter', sans-serif", fontSize: 'clamp(22px, 6vw, 30px)', fontWeight: 600, color: 'var(--fg)', lineHeight: 1.1, letterSpacing: '-0.02em' }}>{p.name}</h3>
        ...
        <div style={{ marginTop: 4 }}><SpecularButton href={p.href} target="_blank" rel="noreferrer" size="sm">Ver projeto ↗</SpecularButton></div>
      </div>
    </div>
  );
}
...
function ProjectsMobile() {
  const head = useReveal<HTMLDivElement>();
  return (
    <section id="trabalhos" style={{ background: '#F8F7F5', padding: '64px 20px 80px' }}>
      <div ref={head.ref} style={{ ...head.style, marginBottom: 32 }}>
        <span style={{ fontSize: 13, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--muted)', display: 'block' }}>01 — Trabalhos selecionados</span>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 12, flexWrap: 'wrap', gap: 12 }}>
          <h2 style={{ margin: 0, fontFamily: "'Inter', sans-serif", fontWeight: 600, fontSize: 'clamp(26px, 7vw, 36px)' }}>Trabalhos em destaque</h2>
          <a href="https://github.com/felippeximenes" target="_blank" rel="noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, border: '1px solid var(--line)', borderRadius: 999, padding: '8px 16px', fontSize: 13, fontWeight: 500 }}>
            Ver todos ↗
          </a>
        </div>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        {projects.map((p) => <ProjectCardMobile key={p.name} p={p} />)}
      </div>
    </section>
  );
}
